...
# Si estás en clase, usa 11_script.R.
...
mapa_municipios <- left_join(
  mapa_geo,
  municipios |> select(clave_municipio, participacion, votos_shh, pct_shh, ventaja_shh),
  by = "clave_municipio"
)
...
municipios_con_judicial <- left_join(municipios, judicial_resumen, by = "clave_municipio")
...
# TRUE. El mismo comando que delató a geom_barplot() como inventada
# aquí confirma que geom_boxplot() sí es real. La comprobación 1 no
# sirve solo para atrapar errores: sirve exactamente igual para
# confirmar que algo está bien, y en los dos casos toma el mismo
# segundo en correr.
