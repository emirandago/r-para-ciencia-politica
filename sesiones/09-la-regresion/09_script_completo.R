...
# Si estás en clase, usa 09_script.R.

# ─────────────────────────────────────────────────────────────
# NOTA DE VERIFICACIÓN PARA EMG — borrar antes de publicar en el sitio.
# Los coeficientes que aparecen en los comentarios de este script (y en
# 09_slides.tex) se calcularon de forma independiente en Python (numpy,
# mínimos cuadrados por matriz normal + error robusto HC1, que es la
# misma cuenta que fixest llama "hetero") sobre el archivo real
# datos/limpios/presidencial_2024_municipio.csv, cruzados con tres
# métodos (ecuación normal, fórmula cov/var y np.polyfit). No se
# corrieron con lm()/feols() en R porque la sesión de RStudio del
# proyecto estaba ocupada durante la producción de este material. Correr
# este script en R debe reproducir exactamente los mismos números:
...
modelo_1 <- lm(ventaja_shh ~ participacion, data = municipios)
...
modelo_3 <- feols(ventaja_shh ~ lista_nominal, data = municipios, vcov = "hetero")

summary(modelo_3)

# El coeficiente sale minúsculo (-0.000034) porque lista_nominal se mide
# en personas, no en puntos porcentuales: un municipio más y un municipio
# menos en la lista no cambian casi nada la ventaja. Para que el número
# se sienta comparable habría que preguntarse cuánto cambia la ventaja
# por, digamos, cada 10 mil personas más en la lista, no por cada persona
# — y ese es exactamente el tipo de decisión de unidades que un
# politólogo tiene que tomar antes de reportar un coeficiente.
