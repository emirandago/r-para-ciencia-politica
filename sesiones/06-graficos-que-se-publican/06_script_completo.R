...
# Si estás en clase, usa 06_script.R.
...
entidad_bloques <- resultados_entidad |>
  select(
    ...
    `Fuerza y Corazón por México` = pct_fcm,
    `Movimiento Ciudadano`        = pct_mc
  ) |>
  pivot_longer(
    cols      = -entidad,      # todo menos entidad se vuelve largo
    names_to  = "bloque",      # el nombre de la columna vieja
    values_to = "porcentaje"   # el valor que traía esa columna
  )
#</hueco>
...
#<hueco>
escala_por_umbrales <- esqueleto +
  scale_fill_steps2(low = "#7B1113", mid = "white", high = "#006847", midpoint = 0) +
  labs(title = "steps2(): divergente, pero por tramos")
#</hueco>
...
#<hueco>
ggsave(
  filename = here("sesiones", "06-graficos-que-se-publican", "figuras",
                   "06_figura_bloques_por_entidad.pdf"),
  plot     = grafico_final,
  width    = 8,
  height   = 9,
  units    = "in",
