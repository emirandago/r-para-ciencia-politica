...
# Si estás en clase, usa 04_script.R.
...
#<hueco>
resumen_nacional <- resultados_entidad |>
  summarise(
    pct_simple    = mean(pct_shh),
    pct_ponderado = 100 * sum(votos_shh) / sum(total_votos)
  )
...
#   con sort = TRUE.
#<hueco>
municipios_por_entidad <- resultados_municipio |>
...
#<hueco>
nombres_repetidos <- resultados_municipio |>
...
cruce_correcto <- resultados_municipio |>
  left_join(puente, by = c("clave_municipio" = "clave_municipio_inegi"))
#</hueco>
